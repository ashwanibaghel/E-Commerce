...
describe('Path safety validation', () => {
  ...
  test('Should normalize paths correctly', async () => {
    const uniqueFile = `normalize-${process.pid}-${Date.now()}.jpg`;
    const testPath = `media//double//slash//${uniqueFile}`;
    const normalizedPath = `media/double/slash/${uniqueFile}`;
    const sourcePath = path.join(ROOTPATH, normalizedPath);
    let result;
    ...
    try {
      ...
      result = await imageProcessor(testPath, 12, 12);
      expect(result).toBeTruthy();
    } finally {
      ...
      await fs.rmdir(path.dirname(sourcePath)).catch(() => {});
      await fs.rmdir(path.dirname(path.dirname(sourcePath))).catch(() => {});
    }
  });
